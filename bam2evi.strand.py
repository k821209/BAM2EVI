...
succ_list = [x.strip() for x in open('mayra_succ_list.v5.txt').readlines()]
nsucc_list = [x.strip() for x in open('mayra_no_succ_list.v5.txt').readlines()]
...
for genename in tqdm(genelist):
    ...
    if strand == '+':
        dic['CDS.depth'].append(float(totdepth_p)/float(length))
        dic['CDS.cov'].append(float(coveredbase_p)/float(length))
    else:
        dic['CDS.depth'].append(float(totdepth_m)/float(length))
        dic['CDS.cov'].append(float(coveredbase_m)/float(length))
    ...
    try:
        ...
        dic['INTRON.num'].append(0)
        dic['INTRON.depth'].append(0)
        dic['INTRON.cov'].append(0)
        bintron = 0
        pass 
    if bintron == 1:
        try:
        ...
        coveredbase_m = 0
        length        = 0 
        dic['INTRON.num'].append(array.shape[0])
        if array.shape[0] > 0:
            for left, right in array:
                length        += right - left + 1 - 1
                contiguity_p   = array_contiguity_p[echr][left-1:right-1] # continuity value require minus 1 from right pos
            ...
            if strand == '+':
                dic['INTRON.depth'].append(float(totdepth_p)/float(length))
                dic['INTRON.cov'].append(float(coveredbase_p)/float(length))
            else:
                dic['INTRON.depth'].append(float(totdepth_m)/float(length))
                dic['INTRON.cov'].append(float(coveredbase_m)/float(length))
        else:
            dic['INTRON.depth'].append(None)
            dic['INTRON.cov'].append(None)
    ...
    if 1:
        ...
        if strand == '+':
            dic['strand'].append(1)
            dic['contiguity'].append(contiguity_p)
            dic['ratio.zero'].append(float(zeropart_p)/float(length))
            dic['total.depth'].append(sum(contiguity_p))
            dic['ratio.depth'].append(float(sum(contiguity_p))/float(length))
            dic['blocks'].append(blocks_p)
            dic['#blocks'].append(len(blocks_p))
            dic['coverage'].append(1-float(zeropart_p)/float(length))
            ...
            dic['strand'].append(0)
            dic['contiguity'].append(contiguity_m)
            dic['ratio.zero'].append(float(zeropart_m)/float(length))
            dic['total.depth'].append(sum(contiguity_m))
            dic['ratio.depth'].append(float(sum(contiguity_m))/float(length))
            dic['blocks'].append(blocks_m)
            dic['#blocks'].append(len(blocks_m))
            dic['coverage'].append(1-float(zeropart_m)/float(length))
...
df_cont.to_pickle('continuity_strand.v5.df')
df_cont.to_csv('continuity_strand.v5.txt',sep='\t')
